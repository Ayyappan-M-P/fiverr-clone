...
//       console.log(response); // Check what response is before accessing response.data
...
const Register = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true);
    try {
      // Check if image URL was successfully generated
      const { url } = await generateImageURL(image);
      console.log("Generated Image URL:", url); // Debug
      if (!url) {
        toast.error('Image URL generation failed.');
        setLoading(false);
        return;
      }
